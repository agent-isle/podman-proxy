# `src/types.rs` — Podman API deserialisation types
...
Also defines the public `SandboxMount` type that describes a host path a sandbox authorizes for container bind mounts, together with the sandbox mount's read-only mode.
